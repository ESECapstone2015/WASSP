...
#define MPU6050_LP_WAKE_5HZ    MPU6050_ LP_WAKE_CTRL_2
...
//The following variables determine the reset button pin and the state of that button
...
*/
  ...
  float accel_angle_z = atan(-1*accel_x/sqrt(pow(accel_z,2) + pow(accel_y,2)))*RADIANS_TO_DEGREES;
  float accel_angle_x = atan(accel_z/sqrt(pow(accel_x,2) + pow(accel_y,2)))*RADIANS_TO_DEGREES;
  
  float accel_angle_y = atan(sqrt(pow(accel_x,2) + pow(accel_z,2))/accel_y)*RADIANS_TO_DEGREES;;
  ...
  // Attempt to compensaste 
  if (gyro_y <= DRIFTTHRESHOLD && gyro_y >= -1*DRIFTTHRESHOLD)
  {
    gyro_angle_y = get_last_y_angle();
  }
  ...
  float angle_y = gyro_angle_y;  //Accelerometer doesn't give z-angle (WHATS THIS FOR?)
