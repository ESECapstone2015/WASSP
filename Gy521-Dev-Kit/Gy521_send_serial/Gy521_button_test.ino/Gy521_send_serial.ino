...
const int buttonPin = 5;  //constant for the button pin  
...
*/
  ...
  */
  ...
  if (gyro_z <= DRIFTTHRESHOLD && gyro_z >= -1*DRIFTTHRESHOLD)
  {
    gyro_angle_z = get_last_z_angle();
  }
  ...
  float angle_z = gyro_angle_z;  //Accelerometer doesn't give z-angle (WHATS THIS FOR?)
...
// OdisHarkinsvisualiser format
  ...
  */
  ...
  if(buttonState == HIGH){ //reset button pressed, reset angles
